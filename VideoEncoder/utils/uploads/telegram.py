...
async def upload_video(message, msg, new_file, filename, c_time, thumb, duration, width, height):
    ...
    if resp:
        await resp._client.send_video(log, resp.video.file_id, thumb=thumb,
...
async def upload_doc(message, msg, c_time, filename, new_file):
    ...
    if resp:
        await resp._client.send_document(log, resp.document.file_id, caption=filename, parse_mode=None)
